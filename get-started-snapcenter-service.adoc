...
You should link:task-signing-up.html[sign up to NetApp Cloud Central], link:task-logging-in.html[log into Cloud Manager], and then set up a link:task-setting-up-cloud-central-accounts.html[NetApp account].

[role="quick-margin-para"]
Only an Account Admin can deploy the SnapCenter Service. However, both Account Admin and SnapCenter Admin can perform different operations. link:reference-user-roles.html[Learn more]
...
[role="quick-margin-para"]
Create an Azure NetApp Files working environment in Cloud Manager to create and manage NetApp accounts, capacity pools, volumes, and snapshots. link:task-manage-anf.html[Learn more]
